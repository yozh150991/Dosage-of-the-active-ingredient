...
test("невідповідна форма завжди супроводжується червоним попередженням", () => {
  let seen = 0;
  for (const p of PRESETS) {
    for (const weightKg of WEIGHTS) {
      ...
      seen++;
      const dangers = buildWarnings(input, d).filter((w) => w.level === "danger");
      assert.ok(dangers.length > 0, p.label + " при " + weightKg + " кг: доза завелика, а попередження немає");
    }
  }
...
test("вік до 3 місяців — червоне попередження для обох препаратів", () => {
  for (const drug of ["paracetamol", "ibuprofen"]) {
    const list = buildWarnings(
      { drug, form: "syrup", weightKg: 5, ageMonths: 2, concMg: 120, concMl: 5 },
      computeDose({ drug, form: "syrup", weightKg: 5, concMg: 120, concMl: 5 })
    );
    const w = list.find((x) => x.code === "age_under_3m");
    assert.ok(w, drug + ": немає попередження про вік");
...
test("вага нижче мінімальної для препарату — червоне попередження", () => {
  assert.ok(warn({ drug: "ibuprofen", form: "syrup", weightKg: 4.5, ageMonths: 4, concMg: 100, concMl: 5 })
    .includes("weight_below_min"));
...
test("таблетки дитині до 6 років — жовте попередження", () => {
  assert.ok(warn({ drug: "paracetamol", form: "tab", weightKg: 16, ageMonths: 48, concMg: 200 })
    .includes("tab_under_6y"));
...
test("нагадування про приховані жарознижувальні є завжди", () => {
  for (const p of PRESETS) {
    const c = warn(Object.assign({ weightKg: 15, ageMonths: 48 }, p));
    assert.ok(c.includes("hidden_paracetamol"), p.label);
  }
});
